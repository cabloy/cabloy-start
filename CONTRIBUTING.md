# Contributing to Cabloy Start

Contributions to Cabloy Start source, tests, tooling, and documentation are welcome. Cabloy Start is a public MIT-licensed Cabloy edition baseline with Vona backend and Zova frontend layers.

## Before you begin

- Use [GitHub Issues](https://github.com/cabloy/cabloy-start/issues) for reproducible bugs, public questions, and feature proposals. Search for an existing issue first.
- Use [pull requests](https://github.com/cabloy/cabloy-start/pulls) for focused, reviewable changes.
- For a bug report, include the expected and actual behavior, a minimal reproduction or sufficient setup details, and relevant Cabloy Start, Node.js, pnpm, database, and browser/runtime versions.
...
CI currently validates Ubuntu with Node.js 24. The project does not publish a formal cross-platform support matrix. For maintained onboarding guidance, see [Cabloy Start](https://cabloy.com/editions/cabloy-start).
...
The tracked pre-commit hook runs `lint-staged`. It may format staged Markdown and configuration files, and may lint-fix and format staged source files. Review the resulting staged changes and run the relevant explicit checks before submitting. Current CI workflows run after pushes to `main`; do not assume that every pull request is automatically validated.
...
For frontend metadata or dependency changes that Vona consumes, build every affected Start flavor before synchronizing dependencies:
...
Canonical public Cabloy documentation is published at [cabloy.com](https://cabloy.com). Discuss proposed public-documentation changes in an issue or pull request rather than treating this repository's copied `repo-docs/` tree as a separate canonical source.

## License and contribution rights

Cabloy Start project-owned content is licensed under the root [MIT License](./LICENSE). [LICENSES.md](./LICENSES.md) explains project-owned license inheritance and third-party notice boundaries.
...
Do not disclose a suspected vulnerability in a public issue or pull request. Cabloy Start does not yet publish a private vulnerability-reporting channel or a `SECURITY.md` policy. Until one is available, use the contact route listed on [cabloy.com](https://cabloy.com) only to establish contact. Do not send exploit details, credentials, tokens, personal data, or other sensitive information through public channels. A dedicated security policy and private reporting route are separate planned work.
