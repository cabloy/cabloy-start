...
| ----------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------- |
| Controller/action tests       | Guard decision, typed scope construction, preflight authorization, and no wire-level authority | Module-local `test/**/*.test.ts` using `performAction(...)` |
| Service/state tests           | Neutral domain lifecycle, supplied-context application, validation, data projection, and invariant behavior | Module-local `test/**/*.test.ts`                            |
...
- an adapter-defined unrestricted subject, ordinary roles with no grant, disabled grant, each supported scope, multiple grants, and custom Department associations;
...
| `ATP-ADM-RES-01`      | Render account `Roles` and `Department Memberships` projections, then issue the visible Department `Move` command and read the refreshed tree/list state.     | The existing Resource owns conventional CRUD/cache state; the visible Department command refreshes the affected Resource state without a full-page reload, while fixture APIs only arrange and clean test data.                                         | Resource, browser      | `SRS-ADM-UI-01`, `SRS-ADM-UI-02`; `WBS-ADM-20-02`, `WBS-ADM-60-03`                                                      |
...
| `ATP-ADM-SSR-01`      | Load anonymous Start Admin SSR, verify the private redirect/login document, sign in, hydrate, and navigate the approved Resource menu routes.                    | SSR redirect/document behavior, hydration, and Resource-menu admission are correct; direct API authorization remains independently covered by API ATP evidence.                                                                                         | SSR, browser           | `SRS-ADM-AUT-*`, `SRS-ADM-UI-03`; `WBS-ADM-20-02`, `WBS-ADM-70-02`                                                      |
...
| `ATP-ADM-POL-02`      | Exercise adapter-defined unrestricted admission for an opted-in RBAC action, no grant, disabled grant, valid grant, protected policy administration, and revision invalidation. | The configured unrestricted subject receives a request-local `all` decision without policy resolution; ordinary missing/disabled policy denies; valid restricted decisions admit only their action; protected control-plane authority remains protected; committed changes do not serve stale policy. | API, transaction | `PRD-ADM-POL-02`, `PRD-ADM-POL-04`, `PRD-ADM-SCP-01`; `SRS-ADM-POL-04`–`SRS-ADM-POL-08`; `WBS-ADM-80-02` |
...
| `ATP-ADM-SCP-02`      | Exercise opted-in Student and Record Controller/action resolution for the adapter-defined unrestricted subject and ordinary delegated scopes, scoped list/view/create/update/delete/nested APIs, and the ids-only `DELETE /training/student/bulk` and `DELETE /training/record/bulk` commands with forged owner/Department/instance fields and out-of-scope IDs. | The guard stores the effective allowed decision before argument extraction; `all` provides active-instance full-row scope and ordinary decisions remain constrained. Student create stamps server ownership, Student update preserves it, Record create inherits ownership from Student, and summary/force-delete/bulk-delete resolve inherited `view`/`delete` policy keys. Empty, duplicate, missing, mixed-scope, and out-of-scope bulk targets fail before mutation; nested Record scope follows Student; client fields cannot widen access. Record nested and multi-row operations intentionally do not claim transaction rollback or row-lock guarantees. | Controller, service, API | `PRD-ADM-SCP-04`, `PRD-ADM-SCP-05`; `SRS-ADM-SCP-06`–`SRS-ADM-SCP-13`; `WBS-ADM-80-03` |
| `ATP-ADM-POL-03`      | Render the Role-detail policy editor through Admin SSR and hydration; create a narrow grant, read it back through the configured Role surface, and verify Role isolation. | The browser-visible editor keeps the grant body narrow, survives SSR/hydration, and does not leak one Role's grant into another Role. Forged or stale capability/projection authority remains covered by dedicated controller/API regression tests. | SSR, browser | `PRD-ADM-POL-01`, `PRD-ADM-SCP-04`; `SRS-ADM-POL-09`, `SRS-ADM-SCP-09`; `WBS-ADM-80-04` |
...
# Focus one current Admin acceptance scenario
npm run test:e2e cabloy-admin -- --grep 'ATP-ADM-POL-03'
```

`test:e2e` is the clean managed-local runner: it checks the configured local port, resets test state, and drives the Vona development SSR runtime. `test:e2e:fast` skips the reset and may target an externally managed runtime through `E2E_BASE_URL`. Flat spec names, such as `cabloy-admin`, appear before `--`; Playwright options and the runner's repeatable `--tag` filter appear after it. Neither mode replaces the paired build/dependency-sync prerequisite. Do not change shared environment identity or ports to bypass a busy resource.
...
| Dynamic RBAC   | `ATP-ADM-POL-01`–`ATP-ADM-POL-03` and `ATP-ADM-SCP-01`–`ATP-ADM-SCP-02` prove catalog opt-in, default deny, independent guards, five scopes, union, Controller/action scope resolution, typed handoff, neutral-service domain enforcement, training inheritance, and capability/API separation before Phase 80 verification. |
