...
4. ordinary-role versus configured built-in/protected-role separation;
...
| `ATP-ADM-ROL-01` | Create/update/delete ordinary roles and replace ordinary memberships; attempt duplicate names and configured built-in mutations. | Ordinary role lifecycle is active-instance-scoped and atomic; built-in names/memberships remain protected; generic APIs cannot mutate `systemAdmin`. | API, transaction | `SRS-ADM-ROL-*`; `WBS-ADM-30-02` |
...
| `ATP-ADM-CTR-01` | Change Vona contract truth and then a Start Admin frontend reverse input after the modules exist. | Generated consumers refresh without edits; paired Start Admin SSR/REST output builds before `deps:vona`. | Build, contract | `SRS-ADM-API-04`, `SRS-ADM-API-05`; `WBS-ADM-20-03`, `WBS-ADM-70-01` |
...
| Admin runtime | `ATP-ADM-RES-01` and `ATP-ADM-SSR-01` prove resource ownership, Admin browser behavior, and API separation. |
