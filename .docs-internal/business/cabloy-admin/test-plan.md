...
| `ATP-ADM-POL-03`      | Render policy catalog/editor and server-derived row/detail action projections through Admin SSR and browser hydration, then call the direct API with stale or forged projection values. | Only safe metadata and constrained action projections are emitted: legacy booleans where applicable, or `{ key, allowed, matcher }` for RBAC actions, with only normalized row-field/accepted-value rules required for UX rendering. SSR remains hydration-equivalent; projections, capabilities, and menus never authorize a direct request. | SSR, browser, API | `PRD-ADM-POL-01`, `PRD-ADM-SCP-04`; `SRS-ADM-POL-09`, `SRS-ADM-SCP-09`; `WBS-ADM-80-04` |
