export default {
  AverageScore: 'Average Score',
  Description: 'Description',
  Operations: 'Operations',
  Student: 'Student',
  SubjectCount: 'Subject Count',
  TotalScore: 'Total Score',
  TrainingRecord: 'Student Training Record',
  RecordController: 'Student Training Record Management',
  RecordCreate: 'Create Student Training Record',
  RecordDelete: 'Delete Student Training Record',
  RecordDeleteBulk: 'Bulk Delete Student Training Records',
  RecordName: 'Training Record Name',
  RecordSelect: 'List Student Training Records',
  RecordUpdate: 'Update Student Training Record',
  RecordView: 'View Student Training Record',
  TrainingRecordName: 'Training Record Name',
  TrainingTime: 'Training Time',
  ScenePhotos: 'Scene Photos',
  DossierFiles: 'Dossier Files',
  AddTrainingRecord: 'Add Student Training Record',
  EditTrainingRecord: 'Edit Student Training Record',
  ViewTrainingRecord: 'View Student Training Record',
  TrainingRecordSubjects: 'Student Training Record Details',
  AddTrainingRecordSubject: 'Add Student Training Record Detail',
  EditTrainingRecordSubject: 'Edit Student Training Record Detail',
  ViewTrainingRecordSubject: 'View Student Training Record Detail',
};
