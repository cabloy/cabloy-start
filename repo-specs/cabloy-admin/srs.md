...
This specification translates the [Cabloy Admin PRD](./prd.md), [ADR 0001](./decisions/0001-admin-mvp-boundaries.md), [ADR 0002](./decisions/0002-dynamic-rbac-and-data-scope.md), and accepted [ADR 0003](./decisions/0003-role-menu-visibility.md) into implementable, testable technical contracts. It is the authority for data and capability ownership, server-side authorization, API and DTO boundaries, transactions, invariants, cache ownership, and contract-loop behavior. The [PDP/WBS](./pdp-wbs.md) sequences delivery; the [test plan](./test-plan.md) defines how these contracts are proved.
...
| `admin-rbac`           | Start role-action grants, custom Department associations, action-policy resolution, catalog projection, action-policy revision/invalidation, and action-policy UI              | Identity, role, Department, Passport facts, or role-menu visibility ownership                     |
| `admin-menu`           | Role-menu associations, menu-visibility revision/invalidation, protected menu catalog/configuration, SSR visibility integration, and the Role-detail Menu Authorization editor | Action grants, Department/owner data-scope resolution, identity, role, or Passport fact ownership |
...
- **SRS-ADM-ROL-04**: Configured built-in role definitions cannot be created, renamed, have their titles or locales changed, or be deleted. Role Management lists and views both `registeredUser` and `systemAdmin` and allows an update only when the patch contains `siteIds`; all other built-in role mutations are rejected server-side.
- **SRS-ADM-ROL-05**: Generic role APIs must additionally reject all `systemAdmin` delete, rename, candidate-selection, and membership-replacement changes. Its `siteIds` site-admission configuration is the sole definition field that Role Management may update. Only the protected workflow can grant or revoke `systemAdmin`.
...
## Role-Menu Visibility Contracts

### Catalog, persistence, and administration

- **SRS-ADM-MNU-01**: The role-menu policy derives a protected catalog from every registered and enabled SSR-site onion in the active runtime. Each partition uses its `ssrSiteName`, such as `start-siteadmin:admin` or `start-siteweb:web`. A declaration binds to a partition using its current SSR-site binding rule: omitted `site` binds to every registered site, while a scalar or array binds only to exact site onion names. Role-menu identity and eligibility are independent of a declaration's `locale`: locale selects display only and does not create a locale-specific association. This does not create an independent site, public path, flavor, tenant, identity, persistence, or API-authority boundary.
- **SRS-ADM-MNU-02**: The catalog derives its hierarchy from actual site-bound menu leaves and groups. A leaf with `roles === undefined` is public and is not configurable. A leaf with defined `roles`, including `roles: []`, is configurable by final leaf identity. Groups are display-only and cannot receive an association.
- **SRS-ADM-MNU-03**: `admin-menu` owns a separate active-instance-scoped role-menu visibility association whose logical identity is `roleId + ssrSiteName + ssrMenuName`, with ordinary entity audit/lifecycle fields and no `enabled` state. A surviving row is the only enabled association; unchecking deletes the exact row atomically and retains no disabled tombstone. `ssrSiteName` is the SSR-site onion name. `ssrMenuName` is the final prepared leaf name: the SSR menu onion name for `item`, or `<ssrMenuName>#<itemKey>` for keyed `items`. It is not an `rbacGrant`, action key, data scope, or authorization decision. The association's tenant identity is derived from trusted role/context resolution, never a DTO field.
- **SRS-ADM-MNU-04**: The protected administration service validates the target role in active-instance scope, exact registered/bound `ssrSiteName`, final configurable leaf identity, and duplicate races without partial writes. It rejects public leaves, groups, unknown/wrong-site/retired identities, and invalid target-role states. It does not reject a target merely because its name is `systemAdmin`; protected mutation admission is independent of visibility evaluation. Tenant-scoped business uniqueness uses ordinary lookup indexes plus transactional service checks, not `table.unique(...)`.
- **SRS-ADM-MNU-05**: Role deletion cleans dependent role-menu associations under the same lifecycle guarantees as existing role-policy records. A missing, renamed, retired, or no-longer-bound site/menu identity is ignored and denied during evaluation, surfaced only to a protected reconciliation path, and never falls back to static, public, title, route, or guessed visibility. Deployment/change management reconciles, migrates, or removes associations when those identities change.
- **SRS-ADM-MNU-06**: A System Administrator alone administers the protected catalog and a target role's configuration. Catalog/configuration DTOs expose only safe site-tree hierarchy, display data, exact site/leaf identity, public/configurable state, and whether the target role has a surviving association; they never expose raw SSR decorator data, static roles, Bean internals, Passport topology, assignments for other roles, API/route permission claims, or public-menu internals.

### SSR evaluation, visibility, and projection

- **SRS-ADM-MNU-07**: The structural SSR menu cache remains keyed by its existing site/instance/host/locale factors. At request time, filtering happens before private menu fields are stripped and distinguishes definedness exactly: `roles === undefined` is public without dynamic evaluation; `roles: []` requires a matching surviving dynamic association; a nonempty array is visible on static any-role match or a matching surviving dynamic association. Structural caching does not retain a subject-specific visible result.
- **SRS-ADM-MNU-08**: Dynamic association matching uses trusted current active-instance role identities plus exact `ssrSiteName` and final `ssrMenuName`; matching surviving rows union across current roles. No Passport, no matching association, a wrong site/menu identity, or a retired identity denies a defined-role leaf unless its nonempty static roles match. `systemAdmin` has no implicit evaluator shortcut.
- **SRS-ADM-MNU-09**: Groups are derived only from visible children; Phase 90 stores no group grant. The existing `GET /api/home/base/menu/{publicPath?}` contract shape and anonymous behavior remain intact. Before serialization, it strips static roles, association state, protected catalog metadata, revisions, and role topology. Ordinary rendered menu names remain available as navigation data; a public DTO cannot reconstruct role associations or private policy state.
- **SRS-ADM-MNU-10**: Menu visibility is navigation disclosure only. A visible, hidden, or guessed menu, route, page, Resource identity, or browser projection never grants a controller, API, action, or data-scope operation. Existing Passport guards, `@Passport.rbac(...)` decisions, Resource permissions, and service authority remain independent server-side boundaries.

### Revisions, session freshness, API, and frontend state

- **SRS-ADM-MNU-11**: A committed role-menu change advances a dedicated menu-visibility revision and invalidates affected site-tree editor configuration queries. Normal `admin-menu.model.roleMenu` writes own entity/query-cache invalidation, including its commit-time invalidation behavior; the SSR visibility resolver uses the Model's normal query/entity cache path rather than setting cache-bypass options. The revision is not an ORM-cache substitute, does not reuse action-RBAC policy revision as the visibility authority, and does not clear `a-permission` action projections as a substitute. Any exceptional direct database maintenance path must invoke the equivalent `roleMenu` cache invalidation explicitly.
- **SRS-ADM-MNU-12**: The next authenticated menu retrieval for a subject reflects committed role-menu policy for the affected site. On the client, a successful role-menu mutation affecting a role held by the current Passport subject invokes `this.app.reload()` after invalidating the affected protected role-configuration state. A successful ordinary-role membership replacement invokes `this.app.reload()` after invalidating the affected User Resource item only when the target is the current Passport subject. Mutations affecting another subject do not reload the current browser. The application reload reconstructs authenticated and navigation state from server-authoritative data; these mutation callbacks do not require a targeted Passport refresh, menu-model refetch, or mutation-level relogin fallback. Phase 90 introduces no real-time cross-browser push.
- **SRS-ADM-MNU-13**: Each site frontend retains its existing menu model ownership and stable public-path/locale query key; it does not add roles, policy revisions, or Passport data to that key. For an affected current subject, application and menu state may be reconstructed through the full reload/startup boundary rather than a direct menu-model refetch. A dedicated `admin-menu` role-menu model owns protected catalog/configuration queries and mutations with distinct query keys. The Role View adds a role-scoped Menu Authorization block beside Resource Permissions while `rest-resource.model.resource` remains the generic Role CRUD/cache owner.
- **SRS-ADM-MNU-14**: Vona entity, DTO, controller, validation, and OpenAPI are contract truth. The implementation generates Zova consumers rather than hand-editing them; Role View metadata and its custom block follow the Start Admin reverse chain: `npm run build:zova:admin` before `npm run deps:vona`.
- **SRS-ADM-MNU-15**: The `admin-menu` role-menu persistence change is folded into its current `vonaModule.fileVersion: 1` schema path without incrementing that value. During this development-stage extraction, the `admin-rbac` version-1 creation path relinquishes the menu tables and `admin-menu` version 1 creates them; existing menu API paths and generated-consumer names need not be preserved. The implementation records the current-version fold and runs `npm run test` after editing the owning `meta.version.ts`.
...
| `PRD-ADM-MNU-*`                 | `SRS-ADM-MNU-*`                                                   | `WBS-ADM-90-01`–`WBS-ADM-90-08`                   | `ATP-ADM-MNU-01`–`ATP-ADM-MNU-09`                                                                                                             |
