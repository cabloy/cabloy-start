...
import { prepareImageFile } from '../../lib/imageTransform.js';
...
export class ControllerImageUploader extends BeanControllerBase {
        ...
        <VCardText class="d-flex flex-wrap align-center ga-3">
          ...
          {state.isUploading && (
            <span class="d-inline-flex align-center ga-2 text-medium-emphasis">
              <VProgressCircular indeterminate size={20} color="primary"></VProgressCircular>
              {this.scope.locale.Uploading()}
            </span>
          )}
          {state.errorMessage && <span class="basis-100 text-error">{state.errorMessage}</span>}
        </VCardText>
      </VCard>
    );
  }
  ...
  private _validateFile(file: File, props: ControllerImageUploaderProps) {
    ...
    const explicitTokens = this._getAcceptTokens(props);
  ...
  private _getAcceptTokens(props: ControllerImageUploaderProps) {
    let accept: string[];
    if (!props.accept) {
      accept = [];
