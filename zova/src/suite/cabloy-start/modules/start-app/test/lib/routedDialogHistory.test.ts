...
test('routed dialog history updates Back availability before navigation listeners', () => {
  ...
  routedDialogHistory.onCanGoBackChange(canGoBack => canGoBackOnChange.push(canGoBack));
  history.listen(() => canGoBackOnNavigation.push(routedDialogHistory.canGoBack));
  history.replace('/entry');
  history.push('/detail');
...
test('routed dialog history truncates forward entries after replacing from the middle', () => {
  const { history, routedDialogHistory } = createHistory();
  history.replace('/entry');
  history.push('/detail/1');
  history.push('/detail/2');
  history.go(-1);
  history.replace('/detail/3');
  history.go(1);
  assert.equal(history.location, '/detail/3');
  assert.equal(routedDialogHistory.canGoBack, true);
  history.go(-1);
...
test('routed dialog histories remain independent and reset after destruction', () => {
  ...
  first.routedDialogHistory.onCanGoBackChange(canGoBack => firstChanges.push(canGoBack));
