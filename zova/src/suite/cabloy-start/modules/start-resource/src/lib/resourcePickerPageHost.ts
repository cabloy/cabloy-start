...
import type { IRoutedDialogContext } from 'zova-module-start-app';
...
export function createResourcePickerPageHost(
  ...
  if (!options || !session) throw new Error('resource picker requires dialog options and session');
  return {
    options,
    session,
    ...
    prepareBlocks: (blocks, pickerContext) => {
      ...
      return blocks.map(block => {
        if (block.render !== 'start-page:blockPage') return block;
        return {
          ...block,
