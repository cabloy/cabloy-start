...
declare module 'zova-module-a-openapi' {
  export interface IResourceFormFieldRecord {
    'start-markdown:formFieldMarkdown'?: IResourceFormFieldMarkdownOptions;
  }
}
...
interface IMarkdownImageUploadSelection extends IMarkdownSelection {
  collapsed: boolean;
}

interface IMarkdownImageUploadMapping {
...
export class ControllerFormFieldMarkdown extends BeanControllerBase {
  ...
  private _codeBlockLanguageSelectFocused = false;
  private _codeBlockLanguageSelectMenuOpen = false;
  ...
  public setCodeBlockLanguageSelectFocused(focused: boolean) {
    this._codeBlockLanguageSelectFocused = focused;
    this._syncCodeBlockToolbarInteracting();
  }

  public setCodeBlockLanguageSelectMenuOpen(menuOpen: boolean) {
    this._codeBlockLanguageSelectMenuOpen = menuOpen;
    this._syncCodeBlockToolbarInteracting();
  }

  private _syncCodeBlockToolbarInteracting() {
    this._codeBlockToolbarInteracting =
      this._codeBlockLanguageSelectFocused || this._codeBlockLanguageSelectMenuOpen;
    this._scheduleContextualToolbarPosition();
  }
  ...
  public beginImageUpload(chooseFiles: () => void) {
    ...
    this._imageUploadTransactionMappings = [];
  ...
  private _resolveImageUploadSelection() {
    const selection = this._imageUploadSelection;
    if (!selection) return undefined;
    return this._imageUploadTransactionMappings.reduce((mapped, mapping) => {
      if (mapped.collapsed) {
        const position = mapping.map(mapped.from, 1);
        return { ...mapped, from: position, to: position };
      }
      return {
        ...mapped,
        from: mapping.map(mapped.from, 1),
        to: mapping.map(mapped.to, -1),
      };
    }, selection);
  }

  private _clearImageUploadSelection() {
    this._imageUploadSelection = undefined;
    this._imageUploadTransactionMappings = [];
  }

  private _resolveImageUrl(url: string | undefined) {
    const resolved = resolveImagePreviewUrl(url, this.sys.config.api.baseURL);
    ...
    this.$controllerMounted(() => {
      ...
      this.editor = new Editor({
        ...
        onTransaction: ({ transaction }) => {
          if (this._imageUploadSelection && transaction.docChanged) {
            this._imageUploadTransactionMappings.push(transaction.mapping);
          }
          this._refreshToolbarState();
        },
        onFocus: () => {
